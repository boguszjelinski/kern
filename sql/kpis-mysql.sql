...
select 'number of request with exceeded detour (>180s = 60s for pickup and 120s tolerance)' AS' ';
select count(*) from taxi_order where distance*60*((100+max_loss)/100) < TIMESTAMPDIFF(second, started, completed) - 180;
select 'number of request with exceeded wait by 1min (>80s)' AS' ';
select count(*) from taxi_order where received is not null and started is not null and TIMESTAMPDIFF(second, received, started) > 60*(max_wait+1)+20;
select 'number of request with exceeded wait by 2min (>140s)' AS' ';
select count(*) from taxi_order where received is not null and started is not null and TIMESTAMPDIFF(second, received, started) > 60*(max_wait+2)+20;
