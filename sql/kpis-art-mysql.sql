...
select (select sum(TIMESTAMPDIFF(second, started, completed)) from taxi_order where completed is not null and started is not null)/(select count(*) from taxi_order where completed is not null and started is not null);
