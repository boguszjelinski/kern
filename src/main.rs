...
mod tests {
  ...
  fn test_performance_lcm() {
    let stops = get_stops(0.05);
    init_distance(&stops);
    let mut orders: Vec<Order> = get_orders2(1900);
    let mut cabs: Vec<Cab> = get_cabs(16000);
    let start = Instant::now();
    let ret2 = lcm_gen_pairs2(&mut cabs, &mut orders, 1800);
    println!("Elapsed: {:?}", start.elapsed()); 
    assert_eq!(ret2.len(), 1800);
  ...
  #[test]  
  #[serial]
  fn test_performance_extern_lcm() {
    let stops = get_stops(0.05);
    init_distance(&stops);
    let mut orders: Vec<Order> = get_orders2(1900);
    let mut cabs: Vec<Cab> = get_cabs(16000);
    let start = Instant::now();
    let ret2 = extern_lcm(&mut cabs, &mut orders, 1800);
    println!("Elapsed: {:?}", start.elapsed()); 
    assert_eq!(ret2.len(), 1800);
   // assert_eq!(ret2[0].0, 901);
   // assert_eq!(ret2[0].1, 1499);
