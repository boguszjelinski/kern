...
use crate::repo::{CNFG,find_legs_by_status,assign_order_find_cab, update_places_in_legs,
                  create_leg,update_leg_a_bit,update_leg_with_route_id, find_routes_by_status};
...
pub fn find_matching_routes(thr_numb: i32, itr: i32, host: &String, client: &mut Client, demand: &Vec<Order>, stops: &Vec<Stop>, 
    ...
    let mut routes: HashMap<i64, i32> = find_routes_by_status(client, RouteStatus::ASSIGNED);
    if legs.len() == 0 {
    ...
    for i in 0..t_numb { 
      ...
      children.push(thread::spawn(move || {
        ...
        for o in start..stop {
            ...
            let feas = try_to_extend_route(&taxi_order, &legs_cpy, &routes_cpy, &bus_stops);
            match feas {
    ...
    for f in feasible.iter() {
      ...
      // first check once again if we have any reserve for that route
      // as one route could be accepted for extension by different threads
      let mut reserve: i32 = match routes.get(&from_leg.route_id) { // both legs have the same route_id
        Some(x) => { *x }
        None => 0
      }; 
      if f.order.from != from_leg.from { // leg will be added
        unsafe {
          reserve -= (DIST[from_leg.from as usize][f.order.from as usize] + DIST[f.order.from as usize][from_leg.to as usize]) as i32 
                                - from_leg.dist;
        }
      }
      if f.order.to != to_leg.to { 
        unsafe {
          reserve -= (DIST[to_leg.from as usize][f.order.from as usize] + DIST[f.order.from as usize][to_leg.to as usize]) as i32 
                                - to_leg.dist;
        }
      } 
...
fn try_to_extend_route(demand: &Order, legs: &Vec<Leg>, routes: &HashMap<i64, i32>, stops: &Vec<Stop>) -> Option<LegIndicesWithDistance> {
    unsafe {
      let mut feasible: Vec<LegIndicesWithDistance> = Vec::new();
      let mut i = check_if_perfect_match_from(demand.from, legs, 1); // we do not start at 0 as there must be at least one leg before 'not astarted', assigned
      ...
      while i < legs.len() {
        ...
        let mut dist_new_path = DIST[leg.from as usize][demand.from as usize] + DIST[demand.from as usize][leg.to as usize];
        // with dist_allowed we get a reserve in the route, here we check maxloss of other orders
        let mut dist_allowed = match routes.get(&leg.route_id) {
          Some(x) => { *x }
          None => 0
        };
        if demand.from != leg.to // == means direct hit in the next leg/iteration, leg.from; here we miss one possibility, where leg is the last leg in that route
            ...
            // check MAXLOSS
            && dist_allowed >= dist_new_path as i32 - leg.dist // new path - old path length; integers so it can even be negative :)
            && ((demand.from == leg.from && demand.wait >= initial_distance as i32 )// direct hit and maxwait check
              || (not_too_long
                    ...
                    && ((dist_new_path + CNFG.stop_wait) as f32) < (leg.dist as f32) * CNFG.extend_margin
        ...
        // to give some time for both parties to get the assignment
        {
          ...
          dist_allowed -= dist_new_path as i32 - leg.dist;
          ...
          while k < legs.len() {
            ...
            dist_new_path = DIST[legs[k].from as usize][demand.to as usize] + DIST[demand.to as usize][legs[k].to as usize];
            if not_too_long
                && dist_allowed >= dist_new_path as i32
                && ((dist_new_path + CNFG.stop_wait) as f32) < (legs[k].dist as f32) * CNFG.extend_margin
...
mod tests {
  ...
  fn test_extend_legs_in_db_returns_sql() {
    let leg = Leg{ id: 1, route_id: 123, from: 1, to: 2, place: 0, dist: 1, started: None, completed: None, status: 0};
  ...
  fn test_update_places() {
    let mut legs: Vec<Leg> = vec![
      Leg{ id: 1, route_id: 123, from: 1, to: 2, place: 0, dist: 1, started: None, completed: None, status: 0},
      Leg{ id: 1, route_id: 123, from: 1, to: 2, place: 1, dist: 1, started: None, completed: None, status: 0}
    ];
  ...
  fn test_try_to_extend_route_when_perfect_match() {
    let legs: Vec<Leg> = vec![
      Leg{ id: 0, route_id: 123, from: 0, to: 1, place: 0, dist: 1, started: None, completed: None, status: 0},
      Leg{ id: 1, route_id: 123, from: 1, to: 2, place: 1, dist: 1, started: None, completed: None, status: 0},
      Leg{ id: 2, route_id: 123, from: 2, to: 3, place: 0, dist: 1, started: None, completed: None, status: 0},
    ];
